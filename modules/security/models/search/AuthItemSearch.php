...
class AuthItemSearch extends AuthItem
{
    ...
    {
        $query = AuthItem::find()
            ->leftJoin( Staff::tableName().' as `create`    on `create`   .`id_profile`    = '.AuthItem::tableName().'.`created_by`')       // Создал
            ->leftJoin( Staff::tableName().' as `update`    on `update`   .`id_profile`    = '.AuthItem::tableName().'.`updated_by`');      // Обновил
