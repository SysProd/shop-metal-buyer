...
/*        if ($model->load(Yii::$app->request->post()) && $model->validate($model)) {
                ...
                if (strlen($model->getErrorMessage()) > 0) {
                    \Yii::$app->getSession()->setFlash('error', $model->getErrorMessage());
                        return $this->redirect(['/security/auth-item/create']);
                }
...
//            return $this->redirect(['/security/auth-item/index']);
    ...
    public function actionUpdate($id)
    {
        ...
        if( !Yii::$app->user->can('Root-Admin') && $model->name == 'Root-Admin') {
    ...
    public function actionRemoveItems()
    {
        ...
        if ( ($authItems = AuthItem::findAll($items)) !== null && !empty($items) ){
            ...
            if(!empty($array) && empty($error)){
                Yii::$app->session->setFlash('success', '<h4>'.Icon::show('check-circle', ['class' => 'fa-lg kv-alert-title']).Yii::t('app','Successful').  '</h4> <p> '.Yii::t('app','{nameType} <strong>{fullName}</strong> removed from the system',    ['nameType' => \Yii::t('app', 'Access rights'), 'fullName' => Html::encode($array)]).'.</p>');
//                return true;
            }else if(!empty($array) && !empty($error)){
                Yii::$app->session->setFlash('error',   '<h4>'.Icon::show('times-circle', ['class' => 'fa-lg kv-alert-title']).Yii::t('app','Unsuccessful').'</h4> <p> '.Yii::t('app','{nameType} <strong>{fullName}</strong> not removed from the system',['nameType' => \Yii::t('app', 'Access rights'), 'fullName' => Html::encode($error)]).'.</p>');
                Yii::$app->session->setFlash('success', '<h4>'.Icon::show('check-circle', ['class' => 'fa-lg kv-alert-title']).Yii::t('app','Successful').  '</h4> <p> '.Yii::t('app','{nameType} <strong>{fullName}</strong> removed from the system',    ['nameType' => \Yii::t('app', 'Access rights'), 'fullName' => Html::encode($array)]).'.</p>');
//                return true;
            }else if( empty($array) && !empty($error)){
                Yii::$app->session->setFlash('error',   '<h4>'.Icon::show('times-circle', ['class' => 'fa-lg kv-alert-title']).Yii::t('app','Unsuccessful').'</h4> <p> '.Yii::t('app','{nameType} <strong>{fullName}</strong> not removed from the system',['nameType' => \Yii::t('app', 'Access rights'), 'fullName' => Html::encode($error)]).'.</p>');
//                return false;
    ...
    public function actionDelete($id)
    {
        ...
        if ($authItem->delete()) {
            Yii::$app->session->setFlash('success', '<h4>'.Icon::show('check-circle', ['class' => 'fa-lg kv-alert-title']).Yii::t('app','Successful').  '</h4> <p> '.Yii::t('app','{nameType} <strong>{fullName}</strong> removed from the system',    ['nameType' => \Yii::t('app', 'Access rights'), 'fullName' => Html::encode($authName)]).'.</p>');
        } else {
            Yii::$app->session->setFlash('error',   '<h4>'.Icon::show('times-circle', ['class' => 'fa-lg kv-alert-title']).Yii::t('app','Unsuccessful').'</h4> <p> '.Yii::t('app','{nameType} <strong>{fullName}</strong> not removed from the system',['nameType' => \Yii::t('app', 'Access rights'), 'fullName' => Html::encode($authName)]).'.</p>');
        }
