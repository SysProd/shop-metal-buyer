        ...
        return [
            'access' => [
                ...
                'rules' => [
                    [
                        'allow' => true,
//                        'roles' => [AuthItem::ROLE_Admin, AuthItem::ROLE_UsersGroup, AuthItem::ROLE_UsersAuthor],
                    ]
                ],
            ],
            'verbs' => [
...
/*            'rateLimiter' => [
                'class' => \ethercreative\ratelimiter\IpRateLimiter::className(),
                'rateLimit' => 30, // Максимальное количество разрешенных запросов
                'timePeriod' => 600, // Период времени, в течение которого применяются к ограничению
                'separateRates' => false,
                'enableRateLimitHeaders' => true, // Следует ли возвращать заголовки HTTP, содержащие текущую информацию об ограничениях скорости
                'errorMessage' => Yii::t('app', 'Rate limit exceeded.'),
            ],*/
        ];
    }
...
/*
        // Запретить выводить "Профиль" недоступного для роли "Администраторы"
        elseif(!Yii::$app->user->can(AuthItem::OPR_UpdateUser))
        {
            throw new NotFoundHttpException( \Yii::t('app', 'Page not found.') );
        }

        // Запретить выводить "Профиль" недоступного для роли "Авторы-Групп"
